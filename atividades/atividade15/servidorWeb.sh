...
GRUPO_ID=$(aws ec2 create-security-group --group-name "group_atv_15" --description "grupo_destinado_atividade_15" --output text)

#Definindo politicas do grupo(Liberando portas 22,80)
aws ec2 authorize-security-group-ingress --group-id $GRUPO_ID --protocol tcp --port 22 --cidr 0.0.0.0/0
aws ec2 authorize-security-group-ingress --group-id $GRUPO_ID --protocol tcp --port 80 --cidr 0.0.0.0/0
